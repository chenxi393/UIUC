...
TEST_CASE("KDTree::testSmallerDimVal Testss", "[weight=1][part=1]") {
vector<Point<3>> pts;
KDTree<3> tree(pts);
Point<3> a(1, 2, 3);
Point<3> b(3, 2, 1);

KDTree<3> kdt = KDTree<3>(pts);
KDTree<3> kdt2(kdt);
REQUIRE( kdt.smallerDimVal(a, b, 0) == true);
REQUIRE( kdt2.smallerDimVal(a, b, 0) == true);
