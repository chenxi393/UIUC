...
class KDTree
{
  ...
  private:
     ...
     */
     void Build(int start,int end,vector<Point<Dim>> Points_,KDTreeNode* & R,int dimension);
     void quick_select(int start,int end,int k,vector<Point<Dim>> &Points_,int dimension);
     void copy(const KDTreeNode* other,KDTreeNode* & R);
     void delete_(KDTreeNode* & R);
     Point<Dim> findNearest(const Point<Dim>& query,const KDTreeNode* R,int dimension) const;
};
