...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for(vector<Frequency>::const_iterator i=frequencies.begin();i!=frequencies.end();i++){
    ...
    while((singleQueue.size()+mergeQueue.size())>1){
        ...
        TreeNode *temp=new TreeNode(s1->freq.getFrequency()+s2->freq.getFrequency());
        temp->left=s1;
        temp->right=s2;
