...
class KDTree
{
  ...
  private:
    ...
    void delete_ (KDTreeNode* &R);

     //helper_function  for copy_constructor and =
    void copy(const KDTreeNode* other,KDTreeNode*& R);

    void Build(int start,int end,vector<Point<Dim>>& curPoints,int dimension,KDTreeNode*& R);

    void quick_select(int start,int end,int mid,vector<Point<Dim>>& curPoints,int dimension)const;

    Point<Dim> findNearest(const KDTreeNode* R, const Point<Dim>& query, int dimension) const ;
};
